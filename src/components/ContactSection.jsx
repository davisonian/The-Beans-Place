...
function TiltCard({children, className, href, target, rel}) {
    ...
    // rotateX is reversed [6, -6] so moving the mouse down tips the top of card away from you - makes it look 3D
    ...
    // React treats lowercase names as plain MHTL tags
    ...
    return (
        <Tag
            ...
            // transformPerspective = home deep the 3D looks: smaller exaggerates it
...
function ContactFormInline() {
    // All four fields in one object. These keys must match the "name" props on the inputs below - that's what makes handleChange work.
    const [formData, setFormData] = useState({name: "", email: "", subject: "", message: ""});
    const [status, setStatus] = useState(null); // null | sending | sent | error
    // Fields the user has visited and left. Errors only show for these so
    // The form doesn't turn red before they have typed anything
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // placeholder: sends nothing, jsut fakes 1.2s of network. Swap in real api call later
